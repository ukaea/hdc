...
program cpo_hdc
    ...
    hdc_set(equilibrium, 'time', 1.1d0)
    hdc_set(equilibrium, 'profiles_1d/psi', (/0.1d0, 0.2d0, 0.3d0/))
    ...
    ! we will compare line by line an example ITM/UAL CPO interface
    !UAL subroutine equ2dist(equilibriumin,distsourceout)
    subroutine test_cpos(equilibriumin, distsourceout)

        !UAL use euITM_schemas
        use HDC
        ...
        hdc_resize(distsourceout, source=equilibriumin)
        ...
        do i=1,hdc_get_shape(equilibriumin)(1)
            ...
            hdc_set(distsource_i, 'time', hdc_get(equilibrium_i, 'time'))
            ...
            hdc_set(distsource_i, 'source/profiles_1d/psi', 2 * hdc_get(equilibrium_i, 'profiles_1d/psi'))
            !HDC The alternative, more explicit solutions are
            !HDC it seems that hdc_get_<type> functions will be handy in Fortran
            hdc_set(distsource_i, 'source/profiles_1d/psi', 2 * hdc_get_double(equilibrium_i, 'profiles_1d/psi'))
            ! or
            psi = hdc_get(equilibrium_i, 'profiles_1d/psi')
            hdc_set(distsource_i, 'source/profiles_1d/psi', 2 * psi)
            ...
            hdc_set(distsource_i, 'codeparam/codename', 'equ2dist')
            hdc_set(distsource_i, 'codeparam/codeversion', '1.0')
            hdc_set(distsource_i, 'codeparam/parameters', 'my_code_specific_parameters')
            hdc_set(distsource_i, 'codeparam/output_diag', 'Made in Python')
            hdc_set(distsource_i, 'codeparam/output_flag', 0)
